> **Reference copy.** This file is copied from `volar/docs/reliability.md` for reference. It describes volar's internal reliability and AI tier system — the direct precursor to the miniskill system. It is not a required component of miniskills, and the tier definitions here (Normal/Hazmat/Experimental/Insecure; Glue/Compiler/Cryptography) are volar-specific. The crate paths and file naming conventions apply to volar; adaptation would be needed for other repositories.
>
> This system is expected to apply to the following portal-co repositories once the miniskill system expands to them: `volar`, `dreamcomp`, `jsaw`, and all transitive dependencies under `github.com/portal-co`. The reliability tier system will remain volar-specific; the miniskill CT/IT system (see `MINISKILL.md`) is what generalises across those repos.
